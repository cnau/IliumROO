...
class SystemLogging
  ...
  def self.add_log_entry(msg, source_id = nil, target_id = nil, object_id = nil)
    log.debug {"adding log entry:#{msg}, source:#{source_id}, target:#{target_id}, object:#{object_id}"}
    row_id = UUID.new
    log_row = {}
    ...
    log.debug {"add log id #{row_id.to_guid.to_s}"}
  ...
  def self.remove_log_entry(log_id, source_id = nil, target_id = nil, object_id = nil)
    log.debug {"removing log entry:#{log_id.to_guid.to_s}, source:#{source_id}, target:#{target_id}, object:#{object_id}"}
  ...
  def self.get_log_entry(log_id, source_id = nil, target_id = nil, object_id = nil)
    log.debug {"getting log entry:#{log_id.to_guid.to_s}, source:#{source_id}, target:#{target_id}, object:#{object_id}"}
    ret = {}
    ...
    log.debug {"retrieved log entries:#{ret}"}
  ...
  def self.get_log(log_key = 'all', *columns_and_options)
    log.debug {"getting log entries for #{log_key}, #{columns_and_options}"}
    entries = CassandraDao.get :log, log_key, *columns_and_options
    log.debug {"got log entries #{entries}"}
    entries
  end
  ...
  def self.get_log_count(log_key = 'all', *columns_and_options)
    log.debug {"counting logs for #{log_key}, #{columns_and_options}"}
    count = CassandraDao.count_columns :log, log_key, *columns_and_options
    log.debug {"found #{count} logs in #{log_key}"}
    count
  end
end